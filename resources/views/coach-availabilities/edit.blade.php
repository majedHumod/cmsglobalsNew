...
@section('title', 'تعديل فترة التوفر')
@section('header', 'تعديل فترة التوفر')
...
<div class="space-y-4">
    ...
    <div class="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900" role="status">
        لا يوجد مدرب في القائمة. شغّل
        <code class="mx-1 rounded bg-amber-100 px-1">php artisan tenants:ensure-admin-coach</code>
        ثم عُد لهذه الصفحة، أو أضف دور <strong>coach</strong> لحساب مناسب.
    </div>
    @endif
    @endrole
    ...
    @endphp
    <form method="POST" action="{{ route('coach-availabilities.update', $coachAvailability) }}" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @csrf
        @method('PUT')
            ...
            <div class="md:col-span-2">
                ...
                <select name="user_id" id="user_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @foreach($coaches as $coach)
                        <option value="{{ $coach->id }}" @selected((int) old('user_id', $coachAvailability->user_id) === (int) $coach->id)>{{ $coach->name }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <select name="day_of_week" id="day_of_week" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                @foreach([0 => 'الأحد', 1 => 'الاثنين', 2 => 'الثلاثاء', 3 => 'الأربعاء', 4 => 'الخميس', 5 => 'الجمعة', 6 => 'السبت'] as $value => $label)
                    <option value="{{ $value }}" @selected((int) old('day_of_week', $coachAvailability->day_of_week) === $value)>{{ $label }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label for="capacity" class="block text-sm font-medium text-gray-700">السعة</label>
            <input type="number" name="capacity" id="capacity" min="1" value="{{ old('capacity', $coachAvailability->capacity) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        <div>
            <label for="start_time" class="block text-sm font-medium text-gray-700">من</label>
            <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $formatTime($coachAvailability->start_time)) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        <div>
            ...
            <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $formatTime($coachAvailability->end_time)) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        <div>
            ...
            <input type="number" name="slot_duration_minutes" id="slot_duration_minutes" min="15" value="{{ old('slot_duration_minutes', $coachAvailability->slot_duration_minutes) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        <div>
            <label for="buffer_minutes" class="block text-sm font-medium text-gray-700">فاصل بين المواعيد</label>
            <input type="number" name="buffer_minutes" id="buffer_minutes" min="0" value="{{ old('buffer_minutes', $coachAvailability->buffer_minutes) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        <div class="md:col-span-2">
            <label for="location" class="block text-sm font-medium text-gray-700">الموقع أو رابط الجلسة</label>
            <input type="text" name="location" id="location" value="{{ old('location', $coachAvailability->location) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        <div class="md:col-span-2 flex items-center gap-3">
            <input type="hidden" name="is_active" value="0">
            <input type="checkbox" name="is_active" id="is_active" value="1" @checked(old('is_active', $coachAvailability->is_active)) class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
