...
<div class="mx-auto max-w-3xl space-y-4">
    ...
    <form action="{{ route('notes.store') }}" method="POST" class="admin-card p-5 sm:p-6 space-y-6">
        @csrf

        <div>
            <h2 class="text-sm font-semibold text-tremor-content-strong">معلومات الملاحظة</h2>
            <p class="mt-1 text-sm text-tremor-content">أدخل عنواناً واضحاً ثم اكتب المحتوى.</p>
            <div class="mt-4">
                <label for="title" class="block text-sm font-medium text-tremor-content-emphasis">عنوان الملاحظة *</label>
                <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="عنوان الملاحظة"
        ...
        <div>
            ...
            <div id="editor-container" class="border-x border-b border-tremor-border rounded-b-tremor-default overflow-hidden">
                <div id="editor" contenteditable="true" class="min-h-48 p-4 focus:outline-none focus:ring-2 focus:ring-orange-300" style="direction: rtl;">{{ old('content') }}</div>
                <textarea name="content" id="content-textarea" class="hidden w-full min-h-48 p-4 border-0 focus:outline-none focus:ring-2 focus:ring-orange-300" style="direction: rtl;">{{ old('content') }}</textarea>
            </div>
            @error('content')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
        </div>

        <div class="rounded-tremor-default border border-tremor-border bg-tremor-brand-faint px-4 py-3 text-sm text-tremor-brand-emphasis">
            استخدم عناوين واضحة ونظّم المحتوى بالقوائم عند الحاجة.
        </div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="admin-btn-brand">حفظ الملاحظة</button>
        </div>
    </form>
</div>
